Shader "TextMeshPro/BrushReveal" {
...
Properties {
	...
	_Brushiness ("Brushiness", Range(0, 1)) = 0.65
	...
	_TextBounds ("Text Bounds", Vector) = (0,0,100,20)
}
...
SubShader {
	...
	Pass {
		...
		pixel_t VertShader(vertex_t input)
		{
			...
			output.revealPos = input.vertex.xy;
			return output;
		}
		...
		// Alternative implementation to UnityGet2DClipping with support for softness.
			...
			float2 revealUV = input.atlas.xy;

			float2 brushUV = revealUV;

			brushUV.x += brushUV.y * _StrokeAngle;
			brushUV *= float2(_NoiseScaleX, _NoiseScaleY);

			float n = fbm(brushUV);

			float gradient = revealUV.x;
